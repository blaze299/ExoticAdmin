...
namespace ExoticAdmin
{
    public partial class UserEditWindow : Window
    {
        ...
        public UserEditWindow(User user)
        {
            ...
            {
                // Ha új, akkor kiírjuk az előre beállított clearance értéket
                txtClearance.Text = _user.Clearance.ToString();
            }
        }
        ...
        private void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    // --- JAVÍTVA: isDriver bekerült az új INSERT lekérdezésbe ---
                    if (_user.Id == 0)
                    {
                        query = @"INSERT INTO users 
                                (username, full_name, email, phoneNumber, clearance, license_number, date_of_birth, license_expiry_date, is_verified, password, is_banned, admin_notes, isDriver) 
                                VALUES 
                                (@user, @name, @email, @phone, @clearance, @license, @dob, @expiry, @verified, '$2a$12$K7O8DqM2f8L2/VzQ4W5E.OeB8jG9h1i2k3l4m5n6o7p8q9r0s1t2u', @banned, @notes, @isdriver)";
                    }
                    else
                    {
                        query = @"UPDATE users 
                                ...
                                is_banned=@banned, admin_notes=@notes, isDriver=@isdriver 
                                WHERE id=@id";
                    }
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        // Átadjuk az isDriver-t (ha eddig igaz volt, vagy ha új sofőrt csinálunk)
                        cmd.Parameters.AddWithValue("@isdriver", _user.IsDriver ? 1 : 0);
